...
<!-- Tools-список не задан намеренно: subagent наследует все инструменты
     родительской сессии (включая SendUserFile, если она доступна в
     текущем окружении). Самоограничение (writes только в /tmp, репу не
     трогать) — soft-rule в теле промта. См. https://code.claude.com/docs/en/sub-agents
     для актуальной спеки frontmatter. -->
