...
## notifications.html — штора с подарками (commit 84cc432, наблюдён 2026-06-11)
- Триггер: `#giftsTrigger` (cell .notification-cell.gift-trigger) сверху списка. Аватары через `.avatars-view .avatars-view__stack` со ВСТРОЕННЫМ overlap — три `.avatar.__size-56.__type-image` в одном стеке (НЕ три отдельных аватара рядом). На notifications.html используются `i.pravatar.cc` для аватаров — в офлайн-тестах загружаются как broken-image иконки, на структуру/функционал не влияет.
- Открытие: тап по .notification-cell или по кнопке `[data-action="open-gifts"]` (внутри ячейки). И то, и другое открывают штору; `[data-action="open-gifts"]` matched через `e.target.closest('.button-container')` в делегате.
- Анимация веера: стэйджер по opacity заметен в окно ~250 ms (i=2 → 0.86, i=1 → 0.49, i=0 → 0). К ~500 ms все три на opacity 1. Финальный transform: i=2 rotate -9°, i=1 rotate +7°, i=0 rotate -3° (по матрицам computed transform — синусы 0.156/0.122/0.052). Веер играет на double-rAF после добавления `__visible` (CSS-класс `__entered` добавляется отдельно).
- Accept-all: класс `__accepted` уносит карточки вверх (`y` уходит в -226..-235 за 600 ms) со стэйджером 0→80→160 ms. Title fade-out, `.gift-sheet__final` фейдится с delay 280 ms (CSS `transition: opacity 320ms 280ms`). Конфетти: 28 частиц `<i>` создаются JS на лету (`buildConfetti()`), animation `confetti-burst` 1100ms, цвета из палитры 6 шт. К ~1100 ms все частицы opacity 0 (конец keyframes).
- Закрытие: `closeSheet` снимает все классы, очищает `confetti.innerHTML` и сбрасывает `built=false`. При reopen конфетти пересоздаётся, веер играет заново — проверено.
- Re-open после accept+done: класс `__accepted` снимается в openSheet (`sheet.classList.remove('__accepted')`), но `__entered` лезет до того, как браузер успевает «увидеть» отсутствие `__accepted` — может остаться от прошлого раза. В моём прогоне всё ок (cards opacity=1 после reopen), но если в будущем заметишь, что карточки не появляются на повторном открытии — копать сюда (race между `add('__visible')` и `add('__entered')` через double-rAF, к этому моменту `remove('__accepted')` уже применён).
- Regression check на notifications.html: tabbar (5 иконок) на месте, список 8 ячеек скроллится внутри `.ll-notifs-list` (overflow-y на нём, не на body — window.scrollY=0 после wheel). Табы (Все/Дружбы/...) не сломаны, активный «Все» с оранжевой подложкой.
- Минор: `.gift-sheet__final-title` рендерится с `<br>` посередине, текстконтент склеивается в "Вы приняливсе подарки" без пробела (если когда-то будут парсить текст для assertion — не споткнуться).
- Виз-замечание: панель шторы НЕ перекрывает первую ячейку списка целиком (panelTop ≈ 285 px при viewport 844 px). Бэкдроп затемняет всё выше панели — это by design для bottom-sheet, но именно с ВЫСОКОЙ ячейкой-триггером (height=150) видно, что под шторой угадывается «принять все» — может выглядеть странновато, но не баг.
