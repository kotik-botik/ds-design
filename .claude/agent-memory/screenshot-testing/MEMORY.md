...
## Секция «Знакомства» в messages.html (.ll-pymk) — verified 2026-06-11
- Инлайн-IIFE в конце body (~стр.243). `var KIND` (стр.244) = `'reconnect' | 'vvz'`, переключает текст хедера и кнопку. PEOPLE[6] рендерятся в `.ll-pymk__row` из одного шаблона `card()`.
- reconnect: title «Давно не общались», кнопка label «Написать», class **`button-container __style-primary-on-color`**. vvz: title «Возможно, вы знакомы», «Дружить», class `button-container __style-primary`.
- Карточка `.vvz-card` (data-dismiss-target): `.vvz-card__media`>img(pravatar)+крестик, `.vvz-card__content`>`.vvz-card__title`(имя)+`.vvz-card__subtitle`(«N общих друзей»)+full-width `.button-container`. На 390vp: cardW=160, btnW=136 (full-width внутри content-падингов).
- `.ll-pymk__row`: overflowX:auto, scrollWidth=1032 > clientWidth=390 → горизонтальный скролл работает (programmatic scrollLeft=200 двигается).
- **Чередование по заходам (verified 2026-06-11)**: `localStorage['pymkVisits']` инкрементится на КАЖДУЮ полную загрузку (стр.~252). `KIND = Math.floor((visits-1)/2) % 2 === 0 ? 'vvz' : 'reconnect'` → заходы 1–2 vvz, 3–4 reconnect, 5–6 vvz. Прогон 6 загрузок подряд (один persistent context, чтобы localStorage пережил) дал ровно ожидаемую последовательность; после 6-й загрузки pymkVisits="6". PASS.
- Тест-ловушка: localStorage НЕ чистится сам между загрузками — для чистого старта `localStorage.clear()` один раз, потом считать заходы заново. `page.goto()` в одном context'е сохраняет origin storage; новый context = чистый visit=1.
- Dismiss (components/dismissible.js, делегирован): тап `.vvz-card [data-dismiss]` (крестик) убирает одну карточку 6→5; `[data-dismiss="row"]` («Скрыть») сносит всю секцию (`.ll-pymk` count 1→0, height 274→removed). Оба с ~600ms анимацией перед удалением — ждать waitForTimeout ~700.
- **ЛЕГИБЕЛЬНОСТЬ on-color кнопки (reconnect)**: вопреки названию `__style-primary-on-color` (предполагает цветную/тёмную подложку), на light-grey карточке (cardBg rgb 247,244,242) кнопка резолвится в **white bg rgb(255,255,255) + dark text rgb(46,47,51) + dark border rgb(46,47,51)**. Контраст текст/кнопка отличный (≈13:1, легибельно). НО кнопка bg почти сливается с карточкой (255,255,255 vs 247,244,242 — Δ ~8/канал), читается в основном за счёт тёмного бордера-обводки. Не «washed-out текст», а «почти невидимая граница кнопки» — визуально это outline-кнопка на светлом фоне. vvz `__style-primary` = orange bg rgb(255,119,0)+white text, контрастная и очевидно нажимаемая.
- Драйверы: /tmp/phase1.js (reconnect+dismiss+scroll), /tmp/phase2.js (vvz). Скрины /tmp/reconnect.png, /tmp/vvz.png.
- RE-VERIFY 2026-06-11 (повторный прогон, подтвердилось 1:1): btnBg reconnect = rgb(255,255,255), color rgb(46,47,51), cardBg rgb(247,244,242). Dismiss-анимация ближе к 500ms (waitForTimeout 500 хватило: 6→5, секция→class `__leaving` height:0). KIND переключается только РЕДАКТИРОВАНИЕМ файла (URL-параметра нет). NB: `git status` всегда показывает messages.html как modified — это **pre-existing uncommitted work ветки**, не наша правка; round-trip edit reconnect→vvz→reconnect даёт нулевую дельту (в diff одна `+var KIND='reconnect'` без парного `-`).
