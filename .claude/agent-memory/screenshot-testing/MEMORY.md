...
### Progressive blur 0→80px в bday-шаблоне (наблюдён 2026-06-11, commit 0d20976)
- Структура: `.moment__bday-blur` → 6 × `.moment__bday-blur-step.__b-1..__b-6` + `.moment__bday-blur-tint` сверху. Каждый step — отдельный backdrop-filter слой, активная полоса задаётся `--bday-blur-from/--bday-blur-to` через linear-gradient mask.
- Подтверждённые computed values: __b-1=blur(2px) [0-20%], __b-2=blur(6px) [16-36%], __b-3=blur(14px) [32-52%], __b-4=blur(28px) [48-68%], __b-5=blur(50px) [64-84%], __b-6=blur(80px) [80-100%]. Соседние диапазоны перекрываются на 4 пп (≈20% длины полосы) — этого хватает, чтобы стыки слоёв не считывались глазом.
- Tint: rgba(0,0,0,0.2) с mask-gradient 0→100% (rgba0 @0% / rgb0 @30%) — fade-in затемнения с верхней четверти.
- Визуально: на цветном SVG-фоне с горизонтальными линиями/подписями переход от резкого верха к ватной нижней части плавный, без ступенек. CTA «Поздравить» (DS-orange) сидит на полностью замыленной части фона.
